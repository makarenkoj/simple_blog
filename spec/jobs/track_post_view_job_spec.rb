...
RSpec.describe TrackPostViewJob, type: :job do
  ...
  describe '#perform' do
    it 'creates a PostView record' do
    ...
    it 'increments the posts views_count' do
      expect { described_class.perform_now(post.id, current_user.id, ip_address) }.to change { post.reload.views_count }.by(1)
    end

    it 'does nothing if the post does not exist' do
      expect { described_class.perform_now(-1, current_user.id, ip_address) }.not_to change(PostView, :count)
    end

    it 'works for guest users (user_id is nil)' do
      expect { described_class.perform_now(post.id, nil, ip_address) }.to change(PostView, :count).by(1)
